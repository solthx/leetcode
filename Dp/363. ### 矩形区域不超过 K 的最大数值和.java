...
class Solution {
    public int maxSumSubmatrix(int[][] matrix, int k) {
        ...
        int cur = 0, rowsum=0, res=Integer.MIN_VALUE;
        for ( int l=0; l<col; ++l )
            for ( int r=l; r<col; ++r ){
                rowsum = 0;
                for ( int i=0; i<row; ++i ){
                    TreeSet<Integer> ts = new TreeSet<Integer>();
                    rowsum += (A[i][r] - A[i][l] + matrix[i][l]);
                    // sum[0,i]->a , sum[0,j]->b , j>i,
                    // 找满足 b-a<=k, a>=b-k
                    // 且 b-a最大，即a最小，即最小的，且满足a>=b-k的a
                    Integer a = ts.ceiling(rowsum - k);
                    if ( a!=null )
                        res = Math.max(rowsum - a, res);
                    ts.add(rowsum);
                }
            }*/
        return res;
